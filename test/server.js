...
const app = express();
const port = 8081;
var rawstr = "qq=\\frac{12345}{3}=\\frac{\\begin{bmatrix} a & b_{xyz} & \\frac{abcm^e}{n} \\\\  d^{qp} & e & foo \\end{bmatrix}}{\\begin{bmatrix} a & c \\end{bmatrix}}=\\text{a b}=\\begin{pmatrix} annnn^y & b_{44}  \\\\ 5^5_3 & zzzz  \\end{pmatrix}=\\int_{a}^{b} f(x) \\, dx = \\begin{cases} \\textrm{true,} & \\textrm{if } 0 < x_u < 35^{kmm} \\\\ \\textrm{false,} & \\textrm{otherwise} x^y \\end{cases} = \\oint_V f(s) = \\int_{a}^{b} f(x) \\, dx  =\\sum_{n=1}^{\\infty}n=\\lim_{x \\to \\infty} sin(x)";
// var rawstr = "\\int_{a}^{b}\\begin{bmatrix} m & efghij^n \\\\  \\frac{xssss}{sy} & k \\end{bmatrix} \\,dx";
// var rawstr = "\\frac{tttx}{y} \\textrm{hi ji}";``
const  bboxverts = mathmesh.mathmeshBox(rawstr,1);
...
app.get("/", (req, res) => {
  ...
  var mathmeshImg = new genpic(rawstr,verts,bboxverts);
